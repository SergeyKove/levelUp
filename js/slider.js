...
if ('IntersectionObserver' in window) {
  ...
  function scrollIt(slideToShow) {
    let scrollPos =
      Array.prototype.indexOf.call(slidesElem, slideToShow) *
      (sliderListElem.scrollWidth / slidesElem.length)
  ...
  function showSlide(dir, slides) {
    ...
    if (visibleElem.length > 3) {
      ...
      const newSlide =
        i === 0 ? visibleElem[0].previousElementSibling : visibleElem[0].nextElementSibling
      if (newSlide) {
        scrollIt(newSlide)
      }
    }
...
// const smallSliderButtons = document.querySelectorAll('.js-slider__navigate-item')
